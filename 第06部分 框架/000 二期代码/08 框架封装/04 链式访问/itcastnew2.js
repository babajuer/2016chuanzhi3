...
(function(w){
    ...
    $$.extend($$,{
        ...
        /*不光考虑不同厂商 还考虑不同版本*/
        /* 但是jquery 2.0开始已经不在支持ie8一下
         bootstrap 3.0 也是明确表示不再支持ie8一下
         淘宝已经慢慢不再兼容ie8一下*/
        /*IE6以上*/
    })
    ...
    $$.extend($$,{
        ...
        removeAttr:function (){
            /*arguments :可以保存实参*/
            /*将一个伪数组转为真数组 -- 因为我们马上要用到数组的方法，如果是伪数组根本没法访问数组的方法*/
            /*伪数组 --就是一个特殊格式的json对象*/
            /* var json={1:'苹果',2:'香蕉',3:'菠萝',4:'榴莲',length:4}
             */

            /*什么是伪数组*/
            /*举例法：我们常见的arguments，getElementByTagName  ---集合 -这个集合并不是一个数组
             而是一个伪数组*/
            /*伪数组并不是一个真的数组，所以无法访问数组对象的里面的属性和方法*/
            /*如何将一个伪数组转为真数组*/
            /*3 常用的字符串操作方法 常用的数组方法*/
            ...
            for(var i=0;i<doms.length;i++){
                /*js原生如何去除一个元素的属性 */
                doms[i].removeAttribute(name)
            }
        },
        removeAttrOld:function (context,name) {
            //思路 骨架
            var doms = $$.$all(context)
            /*设置模式 获取模式*/
            if (name) {
    ...
    /*因为$all()这个方法返回的是当前对象 itcast（为了实现链式访问 return this）*/
    /*所以$()这个函数返回的 是一个对象*/
    /*请问 可以访问css这个方法*/
    function $(context){
        return $$.$all(context)
